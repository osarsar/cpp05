...
std::ostream& operator<<(std::ostream &x, RobotomyRequestForm const &obj)
{
    x << obj.getName() << " , RobotomyRequestForm sgrade " << obj.getsGrade() << " , RobotomyRequestForm egrade " << obj.getsGrade() << " , sign = " << obj.getsign() << "target = " << obj.get_target() << std::endl;
    return (x);
}
