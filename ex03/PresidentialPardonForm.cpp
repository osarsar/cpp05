...
std::ostream& operator<<(std::ostream &x, PresidentialPardonForm const &obj)
{
    x << obj.getName() << " , PresidentialPardonForm sgrade " << obj.getsGrade() << " , PresidentialPardonForm egrade " << obj.getsGrade() << " , sign = " << obj.getsign() << "target = " << obj.get_target() << std::endl;
    return (x);
}
